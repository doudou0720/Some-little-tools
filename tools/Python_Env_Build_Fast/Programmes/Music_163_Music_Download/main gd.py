...
input(">>")
k = k.replace(" ","_")
sql = f"""CREATE TABLE IF NOT EXISTS 指定歌单_{k}(
...
url = f"https://music.163.com/#/playlist?id={v}"
driver.get(url)
driver.switch_to.frame("contentFrame")
song_list = driver.find_elements(By.XPATH,'//*[@class="m-table "]/tbody/tr')
for song in song_list:
    ...
    print("execute SQL command:"+f"INSERT INTO 指定歌单_{k} (排名,曲名,歌手,时长,播放链接,歌曲id) VALUES (?,?,?,?,?,?)",rank,name,artist,long,herf,song_id)
    cur.execute(f"INSERT INTO 指定歌单_{k} (排名,曲名,歌手,时长,播放链接,歌曲id) VALUES (?,?,?,?,?,?)",(rank,name,artist,long,herf,song_id))
conn.commit()
cur.close()
conn.close()
